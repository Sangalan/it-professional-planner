"""
Reorganiza el plan de certificaciones 2026 en versión COMPACTA:
sin huecos entre sesiones, examen justo después del repaso final.
Fases 1-6 encadenadas sin pausas; Fase 7 (AWS) mantiene su estructura
de 13 semanas pero empieza en cuanto termina la Fase 6.

Final: 17 oct 2026 (en lugar de 22 nov).
"""
...
# ── 1. ELIMINAR tareas existentes de certificaciones ──────────────────────────
CERT_IDS = [
...
CERT_UPDATES = {
    "cert-dlai-rag": (
        "2026-04-27",
        "Fase 1 (19 abr – 27 abr · 15h) [plan compacto]\n"
        "Punto de control:\n"
    ...
    "cert-ibm-rag": (
        "2026-05-21",
        "Fase 2 (28 abr – 21 may · 45h) [plan compacto]\n"
        "8 cursos + capstone. Plan ~14h/semana (~2h/día).\n"
    ...
    "cert-nvidia": (
        "2026-06-14",
        "Fase 3 (22 may – 14 jun · 35h) [plan compacto]\n"
    ...
    "cert-neo4j-prof": (
        "2026-06-23",
        "Fase 4 (15 jun – 23 jun · 15h) [plan compacto]\n"
    ...
    "cert-neo4j-gds": (
        "2026-07-05",
        "Fase 5 (24 jun – 5 jul · 20h) [plan compacto]\n"
    ...
    "cert-ontotext": (
        "2026-07-18",
        "Fase 6 (6 jul – 18 jul · 25h) [plan compacto]\n"
    ...
    "cert-aws-sap": (
        ...
        "Fase 7 (19 jul – 17 oct · 100h · 13 semanas) [plan compacto]\n"
        "Estructura intacta (el plan original recomienda no tocar el buffer de AWS SAP):\n"
        "- Semanas 1-7 (19 jul – 5 sep): curso core ~50h "
        "(Adrian Cantrill o Stephane Maarek)\n"
        "- Semanas 8-10 (6 sep – 26 sep): hands-on AWS Free Tier + AWS SimuLearn ~25h\n"
        "- Semanas 11-12 (27 sep – 10 oct): simulacros Tutorials Dojo "
        "(mínimo 3 completos) ~20h\n"
        "- Semana 13 (11 oct – 17 oct): examen oficial de práctica AWS + "
        "revisión ligera. Examen real ~14 oct.\n\n"
        "Recordatorio: reservar el examen el 4 sep "
        "(~5-6 semanas de antelación).\n\n"
        "Punto de control 1 (2026-09-06): completar el Practice Question Set oficial.\n"
        "Punto de control 2 (2026-09-27): llegar al simulacro 3 con >=80%.\n"
...
# ════════════════════════════════════════════════════════════════════════════════
new_tasks += [
    # Reservar examen el primer día de la fase
...
# ════════════════════════════════════════════════════════════════════════════════
# FASE 7 · AWS Solutions Architect Professional (19 jul – 17 oct · 13 semanas)
# Mantiene la estructura original (cada 2 días en core, etc.) pero adelantada.
# ════════════════════════════════════════════════════════════════════════════════

# Reserva del examen (~5.5 semanas antes del examen del 14 oct)
new_tasks.append(
    task("task-aws-sap-book",
         "Reservar examen AWS Certified Solutions Architect - Professional",
         "2026-09-04", "cert-aws-sap", "AWS exam booking", "17:30", "18:00")
)

# Semanas 1-7: curso core (19 jul – 5 sep · ~50h), sesión cada ~2 días
aws_core = [
    ("task-aws-sap-01", "AWS SAP-C02 - Guía oficial y diagnóstico inicial",                       "2026-07-19"),
    ("task-aws-sap-02", "AWS SAP-C02 - Dominio 1: Organizations, SCPs y cuentas",                  "2026-07-21"),
    ("task-aws-sap-03", "AWS SAP-C02 - Dominio 1: IAM Identity Center y federación",               "2026-07-23"),
    ("task-aws-sap-04", "AWS SAP-C02 - Dominio 1: logging, auditoría y governance",                "2026-07-25"),
    ("task-aws-sap-05", "AWS SAP-C02 - Dominio 1: Transit Gateway, DX y VPN",                     "2026-07-27"),
    ("task-aws-sap-06", "AWS SAP-C02 - Dominio 1: DNS híbrido y conectividad inter-región",        "2026-07-29"),
    ("task-aws-sap-07", "AWS SAP-C02 - Dominio 2: cómputo resiliente",                            "2026-07-31"),
    ("task-aws-sap-08", "AWS SAP-C02 - Dominio 2: datos y almacenamiento a escala",                "2026-08-02"),
    ("task-aws-sap-09", "AWS SAP-C02 - Dominio 2: integración y mensajería",                      "2026-08-04"),
    ("task-aws-sap-10", "AWS SAP-C02 - Dominio 2: serverless y event-driven",                     "2026-08-06"),
    ("task-aws-sap-11", "AWS SAP-C02 - Dominio 2: seguridad y cifrado",                           "2026-08-08"),
    ("task-aws-sap-12", "AWS SAP-C02 - Dominio 2: coste y rendimiento",                           "2026-08-10"),
    ("task-aws-sap-13", "AWS SAP-C02 - Dominio 3: observabilidad y operaciones",                  "2026-08-12"),
    ("task-aws-sap-14", "AWS SAP-C02 - Dominio 3: alta disponibilidad y disaster recovery",        "2026-08-14"),
    ("task-aws-sap-15", "AWS SAP-C02 - Dominio 3: backup, RPO/RTO y continuidad",                 "2026-08-16"),
    ("task-aws-sap-16", "AWS SAP-C02 - Dominio 3: performance tuning",                            "2026-08-18"),
    ("task-aws-sap-17", "AWS SAP-C02 - Dominio 3: cost optimization y FinOps",                    "2026-08-20"),
    ("task-aws-sap-18", "AWS SAP-C02 - Dominio 4: discovery y estrategia de migración",            "2026-08-22"),
    ("task-aws-sap-19", "AWS SAP-C02 - Dominio 4: migración de bases de datos",                   "2026-08-24"),
    ("task-aws-sap-20", "AWS SAP-C02 - Dominio 4: modernización con contenedores",                "2026-08-26"),
    ("task-aws-sap-21", "AWS SAP-C02 - Dominio 4: modernización serverless y eventing",           "2026-08-28"),
    ("task-aws-sap-22", "AWS SAP-C02 - AWS Well-Architected Tool hands-on",                       "2026-08-30"),
    ("task-aws-sap-23", "AWS SAP-C02 - Architecting on AWS repaso dirigido",                      "2026-09-01"),
    ("task-aws-sap-24", "AWS SAP-C02 - Advanced Architecting on AWS repaso dirigido",             "2026-09-03"),
]
for tid, title, d in aws_core:
    new_tasks.append(task(tid, title, d, "cert-aws-sap"))

# Semanas 8-10: hands-on + primeros simulacros (6 sep – 26 sep · ~25h)
aws_handson = [
    ("task-aws-sap-25",           "AWS SAP-C02 - Practice Question Set oficial (punto de control 1)",         "2026-09-06"),
    ("task-aws-sap-26",           "AWS SAP-C02 - Builder Labs y casos prácticos multi-cuenta",                "2026-09-08"),
    ("task-aws-sap-27",           "AWS SAP-C02 - Whiteboard enterprise multi-region",                        "2026-09-10"),
    ("task-aws-sap-28",           "AWS SAP-C02 - Whiteboard seguridad, red y compliance",                    "2026-09-13"),
    ("task-aws-sap-29",           "AWS SAP-C02 - Whiteboard migración y modernización",                      "2026-09-15"),
    ("task-aws-sap-handson-extra","AWS SAP-C02 - Hands-on AWS SimuLearn: escenarios avanzados",              "2026-09-17"),
    ("task-aws-sap-30",           "AWS SAP-C02 - Simulacro 1 por dominios (Tutorials Dojo)",                 "2026-09-20"),
    ("task-aws-sap-31",           "AWS SAP-C02 - Revisión de errores y flashcards (simulacro 1)",            "2026-09-22"),
    ("task-aws-sap-32",           "AWS SAP-C02 - Simulacro 2 y preguntas difíciles (Tutorials Dojo)",        "2026-09-24"),
]
for tid, title, d in aws_handson:
    new_tasks.append(task(tid, title, d, "cert-aws-sap"))

# Semanas 11-12: simulacros finales (27 sep – 10 oct · ~20h)
aws_mocks = [
    ("task-aws-sap-revision2",    "AWS SAP-C02 - Revisión de errores simulacro 2 + lagunas críticas",        "2026-09-27"),
    ("task-aws-sap-simulacro3",   "AWS SAP-C02 - Simulacro 3 completo (Tutorials Dojo)",                     "2026-09-30"),
    ("task-aws-sap-revision3",    "AWS SAP-C02 - Revisión errores simulacro 3: redes, migración, multicuenta","2026-10-02"),
    ("task-aws-sap-practica-aws", "AWS SAP-C02 - Examen oficial de práctica AWS (punto de control 2)",        "2026-10-05"),
    ("task-aws-sap-consolidacion","AWS SAP-C02 - Consolidación servicios clave y revisión de lagunas",        "2026-10-08"),
]
...
labels = {
    ...
    "cert-aws-sap":    "Fase 7 · AWS SAP          (19 jul – 17 oct · 91 d)",
}
for c in order:
